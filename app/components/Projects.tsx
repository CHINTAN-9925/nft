...
const Projects: React.FC = () => {
    return (
        ...
        {/* <div className="flex h-[430px] mt-[37px]  text-black space-x-[24.42px]"> */}
            {projectData.map(project => (
                <div key={project.id} className="w-[338.46px] h-[323.63px] bg-white rounded-3xl mt-[31.16px] ml-[108px]">
                    <Image className='ml-[28px] mt-[35.84px]' src={project.image} width={project.imgWidth} height={project.imgHeight} alt={project.title} />
                    <div style={{ width: project.titleWidth }} className='text-sky-700 text-[22px] font-semibold font-poppins h-[33px] ml-[27.92px] mt-[14.94px]'>{project.title}</div>
                    <div className='w-[282.63px] h-[78.51px] text-neutral-400 text-sm font-normal font-poppins leading-[30px] mt-[7.13px] ml-[27.92px] '>{project.description}</div>
                    <div className='flex'>
                        <div className='w-[89px] h-[16px] lesding-[24px] text-sky-500 text-base font-semibold font-poppins ml-[27.92px] mt-[20.27px]'>Read more</div>
